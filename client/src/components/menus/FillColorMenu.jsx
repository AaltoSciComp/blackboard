...
import { COLOR_OPTIONS, DEBUG_LEVELS } from "../../constants.js";
import { getUsedColors } from '../../Utils.js';
import { log } from '../../logging.js';


const FillColorMenu = (props) => {
    ...
    useEffect(() => {
        async function cb() {
            const clrs = await getUsedColors('fill', props.sid).catch(err => displayError(err + ' while getting colors used'));
            if(Array.isArray(clrs?.colors)) setFillColors(clrs?.colors);    
        }
        if (props.sid > 0) cb();
    }, [props.sid]);
    ...
    /*const handleShapeColorChange = useCallback(async (mode, color, save=false) => {
        ...
        if(save) {
            const clrs = await getUsedColors('fill').catch(err => displayError(err + ' while getting colors used'));
            if(Array.isArray(clrs.colors)) setFillColors(clrs.colors);

            // Remove duplicates from used colors
            if(!fillColors.includes(color.color)) {
                //console.info(color + ' not included yet')
                const colors = fillColors.concat(color.color);
    ...
    const menuIcon = useCallback((m) => {
        if(props.colorProp.color === 'wipe') {
            return <Icon.DashSquareFill color={props.boardColor}/>;
        } else {
            if(colorDisabled) {
                return <Icon.XSquare color={props.colorProp.color} />;
            } else {
                return <Icon.SquareFill color={props.colorProp.color} />;
            }
        }
    ...
    const usedShapeColors = fillColors.map((color) =>
    ...
    const handleToggle = (e) => {
        if(menuOpen){
            ...
            props.handleShapeColorChange('fill', {color: props.colorProp.color, enabled: !colorDisabled}, true);
    ...
    const handleSave = (a, close=false) => {
        props.handleShapeColorChange('fill', {color: a, enabled: !colorDisabled});
        if(close) handleToggle();
    }

    const toggleColorDisabled = (e) => {
        props.handleShapeColorChange('fill', {color: props.colorProp.color, enabled: !e});
        setColorDisabled(e);
    }

    const openEyeDropper = (e) => {
        if (typeof window === 'undefined' || !('EyeDropper' in window)) {
            log(DEBUG_LEVELS.INFO, 'Your browser does not support the EyeDropper API');
            return;
        }
        ...
        eyeDropper.open(/*{ signal: abortController.signal }*/).then(result => {
            ...
            if(color) {
                props.handleShapeColorChange('fill', {color: color, enabled: !colorDisabled});
            }
        }).catch(e => {
            log(DEBUG_LEVELS.ERROR, 'Error in openEyeDropper: ' + e);
        });
    }
    
    return (
    <Dropdown show={menuOpen} onToggle={handleToggle} size={props.wideUI ? "" : "sm"}>
        <Dropdown.Toggle size={props.wideUI ? "" : "sm"} title={props.mode === 'Fill' ? 'Set fill color' : 'Set line color'}>
            {props.wideUI ? 'Fill c' : null} {menuIcon(props.mode)} {/*<span key={'toggle-' + props.mode} className={colorDisabled ? 'colorbox disabled' : 'colorbox'} style={{ height: '16px', backgroundColor: (props.colorProp.color === 'wipe' ? 'transparent' : props.colorProp.color) }}>{props.colorProp.color === 'wipe' && <Icon.PatchMinus />}</span>*/ }
        </Dropdown.Toggle>
        <Dropdown.Menu rootCloseEvent="pointerdown" style={{minWidth: "232px", backgroundColor: props.boardColor}} className={darkMenu ? "darkmenu" : null}>
            ...
            <Form.Label className={darkMenu ? "text-light ms-1" : "ms-1"}>{props.mode === 'Fill' ? 'Fill color:' : 'Line color:'} {colorDisabled ? 'none' : props.fill.color}</Form.Label>
            { props.ui?.complex && <Form.Check type="checkbox" id={'disable-' + props.mode} label={'Disable fill'} onChange={(e) => (toggleColorDisabled(e.target.checked))} checked={colorDisabled || false}></Form.Check>}
...
const MemoizedFillColorMenu = React.memo(FillColorMenu);
export default MemoizedFillColorMenu;
